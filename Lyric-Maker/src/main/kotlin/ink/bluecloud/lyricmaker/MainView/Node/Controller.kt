...
import CloudNodes.CloudMessageBox.Property.MessageBoxProperty
                ...
                children.run {
                    ...
                    add(CloudButton(theme = ButtonBlueTheme(), property = ButtonProperty("\uE6DF").apply { textFont.set(font) }).apply {
                        setOnMouseClicked {
                            ...
                            settingView.runLock.set(CloudMessageBox(title = "设置", button = arrayOf( "确定", "重置", "取消"), node = settingView, property = MessageBoxProperty().apply { icon.set(args.stage.icons[0]) }).showAndGet())
                        }
                    })
